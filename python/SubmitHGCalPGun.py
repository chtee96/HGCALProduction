...
def parseOptions(parser=None, opt=None):
    ...
    if (opt.CONFIGFILE == '' and opt.DTIER != 'ALL'):
        opt.CONFIGFILE = 'partGun_'+opt.DTIER+'_template.py'
    else:
        opt.CONFIGFILE = 'partGun_GSD_template.py'
...
def submitHGCalProduction(*args, **kwargs):
    ...
    if opt.outDir:
        ...
        if not DASquery:
            ...
            if opt.TAG and opt.DTIER == 'RECO':
                outDir = opt.inDir.strip("/") + "_" + opt.TAG
                print(opt.inDir)
                print(opt.TAG)
                print(outDir)
            #If option is NTUP directory name too large.  
            elif opt.TAG and opt.DTIER == 'NTUP':
                outDir = opt.inDir.strip("/")
            else:
                outDir = opt.inDir
        else:
  ...
  # prepare dir for GSD outputs locally or at EOS
    ...
    if (opt.DTIER == 'RECO' or opt.DTIER == 'NTUP'):
        ...
        if not DASquery:
            regex = re.compile(r"{prefix}(_\S*)_x([0-9]*)(_\S*){tier}_([0-9]*).root".format(prefix=commonFileNamePrefix, tier=previousDataTier))
    ...
    for job in range(1,int(njobs)+1):
        ...
        outhgcfile = basename.replace(dtier, '%s_hgc' %(dtier) ) +'.root'
        ...
        s_template=s_template.replace('DUMMYHGCFILENAME',outhgcfile)
        ...
        if (opt.DTIER == 'ALL'):
            ...
            sr_template=sr_template.replace('DUMMYHGCFILENAME',outhgcfile)
            ...
            sn_template=sn_template.replace('DUMMYHGCFILENAME',outhgcfile)
        ...
        write_condorjob.write('executable  = '+currentDir+'/../scripts/SubmitFileGSD.sh \n')
